...
const WOSubmittedModal: React.FC<WOSubmittedModalProps> = ({
    ...
    const handleViewWorkOrderList = () => {
        ...
        sessionStorage.setItem('newWorkOrder', JSON.stringify({
                ...
                <div className="space-y-4 text-sm text-gray-700">
                    <div className="border p-4 rounded-md">
                        ...
                        <div className="flex mb-2">
                            ...
                            <span className="ml-2">{data.description || "N/A"}</span>
                        </div>
                            ...
                            <div className="ml-2 flex space-x-2">
                                {filePreviews.length > 0
                                    ? filePreviews.map((file, index) => (
                                          <img key={index} src={file.url} alt={file.name} className="w-20 h-20 object-cover rounded inline-block" />
                                      ))
                                    : "N/A"}
                            </div>
                        </div>
